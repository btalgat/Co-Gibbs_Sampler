...
def multi_co_weight(coord, model, cc, nugget, krigtype):
    ...
    # coordtail = np.tile(coord.T, ndata).T
    # coordhead = np.repeat(coord, ndata, axis=0)
    # h = coordtail - coordhead
    # ha = np.sqrt(np.sum(np.power(h, 2), 1))
    # ha = np.resize(ha, [ndata, ndata])
